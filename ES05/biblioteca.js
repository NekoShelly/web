...
function cercaPerAutore(libri, autore) {
  ...
  let l=libri.filter(libro=>libro.autore==autore);
  return l;
}
...
function libriDisponibili(libri) {
  ...
  let l=libri.filter(libro=>libro.disponibile==true);
  return l;
}
...
function stampaBiblioteca(libri) {
  ...
  for(let i=0;i<libri.length;i++){
      console.log("Titolo: "+libri[i].titolo);
      console.log("Autore: "+libri[i].autore);
      console.log("Anno: "+libri[i].anno);
      if(libri[i].disponibile==true){
          console.log("Disponibile: Sì");
      } else {
          console.log("Disponibile: No");
      }
      console.log();
  }
}
...
function statisticheBiblioteca(libri) {
  ...
  let pag=libri.reduce((s, libro) => {
    return s+libro.pagine;
  }, 0);


  let a=[...new Set(libri.map(libro=>libro.autore))];
  
  return {
    totaleLibri: libri.length,
    libriDisponibili: disp,
    totalePagine: pag,
    mediaPagine: Math.round(pag/libri.length),
    autori: a
  };
}
